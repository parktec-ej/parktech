...
export async function GET(req: NextRequest) {
  try {
    ...
    const summary = {
      total: rows.length,
      succeeded: rows.filter((x) => x.refundStatus === "SUCCEEDED").length,
      failed: rows.filter((x) => x.refundStatus === "FAILED").length,
      notRequired: rows.filter((x) => x.refundStatus === "NOT_REQUIRED").length,
      pending: rows.filter((x) => x.refundStatus === "PENDING").length,
      none: rows.filter((x) => x.refundStatus === "NONE").length,
      refundTotal: rows.reduce((sum, x) => sum + (x.refundAmount ?? 0), 0),
    };
