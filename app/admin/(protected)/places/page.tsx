...
type SpotItem = {
  ...
  label: string | null;
  isActive: boolean;
  operationModeOverride: null | OperationMode;
};
...
export default function AdminPlacesPage() {
  ...
  const [operationMode, setOperationMode] = useState<OperationMode>("RESERVATION_THEN_HOURLY");
  ...
  async function savePlaceMode(placeId: string) {
    ...
    try {
      ...
      setMsg(`Place の営業モードを保存しました: ${json.place?.operationMode ?? ""}`);
      await loadPlaces();
    } catch (e: any) {
      setErr(String(e?.message ?? e));
    } finally {
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <div>
              ...
              >
                ...
                <option value="CLOSED">営業しない</option>
              </select>
            </div>
          </div>
              ...
              >
                ...
                <div style={{ marginTop: 14, display: "grid", gap: 10 }}>
                  {place.spots.map((spot) => {
                    ...
                    const effectiveMode =
                      currentValue || (placeModes[place.id] ?? place.operationMode);
